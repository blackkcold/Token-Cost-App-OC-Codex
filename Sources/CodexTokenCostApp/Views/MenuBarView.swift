...
struct MenuBarView: View {
    ...
    private func cardBarColor(for pct: Double) -> Color {
        if appPreferencesModel.preferences.balanceDisplayMode == .remaining {
            // Remaining mode: high remaining = green, low remaining = red
        ...
        // Used mode: low usage = green, high usage = red
        if pct < 0.5 { return gradientColor(for: .low) }
    ...
    private func statusIndicator(
        label: String, hasPayload: Bool, isBusy: Bool, palette: TokenCostPalette
    ) -> some View {
        HStack(spacing: 4) {
            Circle()
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 10) {
                statusIndicator(
                    label: openCodeModel.statusMessage,
                    hasPayload: openCodeModel.selectedPayload != nil,
                    isBusy: openCodeModel.isRefreshing || openCodeModel.isBootstrapping,
                    palette: palette
                )
                statusIndicator(
                    label: codexModel.statusMessage,
                    hasPayload: codexModel.payload != nil,
                    isBusy: codexModel.isRefreshing || codexModel.isBootstrapping,
                    palette: palette
                )
            }
        }
